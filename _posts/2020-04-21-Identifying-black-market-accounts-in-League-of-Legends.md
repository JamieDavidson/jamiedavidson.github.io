...
* Use Riot's API docs to make a plan of action and decide what queries we need to make.
...
### Use Riot's API to make a plan of action and discover what queries we need to make
...
The Riot Games API splits data by geographical region when data is requested from the API. I play primarily on the EU West server, so the data that I'll be retrieving will come from `euw1.api.riotgames.com`.
...
Something that I haven't seen yet is the rune page and summoner spells used by the player in a single match. This information is not returned from the match history endpoint, but is displayed in a league of legends player's match history screen.
...
The chunks that I'm interested in are firstly the participant entry for the player we're investigating:
```javascript
{
    "player": {
